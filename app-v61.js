...
function actualizarGraficoCapeCin(h, etiquetas, modelo) {
  ...
  if (fuenteTexto) fuenteTexto.textContent = `Fuentes: CAPE ${fuenteCape} · CIN ${fuenteCin}. Escala cualitativa propia de MetVlc.`;
  ...
  const maxY = Math.max(1200, Math.ceil(capeMax / 100) * 100);
  const minY = Math.min(-100, Math.floor(cinMin / 25) * 25);
  ...
  crearGrafico('capeCin', 'graficoCapeCin', {
    ...
    options: {
      ...
      plugins: {
        ...
        bandasOperativas: {
          bandas: [
            { desde: minY, hasta: -100, etiqueta: 'CIN fuerte', color: 'rgba(145,71,34,.08)' },
            { desde: -100, hasta: -50, etiqueta: 'CIN moderada', color: 'rgba(188,112,44,.08)' },
            { desde: -50, hasta: -25, etiqueta: 'CIN débil', color: 'rgba(218,157,73,.08)' },
            { desde: -25, hasta: 0, etiqueta: 'Poca inhibición', color: 'rgba(239,205,126,.08)' },
            { desde: 0, hasta: 100, etiqueta: 'CAPE muy baja', color: 'rgba(96,155,109,.06)' },
            { desde: 100, hasta: 300, etiqueta: 'CAPE baja', color: 'rgba(116,177,110,.07)' },
            { desde: 300, hasta: 700, etiqueta: 'CAPE moderada', color: 'rgba(226,184,72,.08)' },
            { desde: 700, hasta: 1200, etiqueta: 'CAPE alta', color: 'rgba(226,125,55,.08)' },
            { desde: 1200, hasta: maxY, etiqueta: 'CAPE muy alta', color: 'rgba(199,67,57,.08)' }
          ]
        }
      },
      scales: {
        ...
        y: {
          ...
          afterBuildTicks: escala => {
            const candidatos = [minY, -100, -50, -25, 0, 100, 300, 700, 1200, maxY];
            escala.ticks = [...new Set(candidatos.filter(v => v >= minY && v <= maxY))].sort((a,b) => a-b).map(value => ({ value }));
          },
          ticks: {
            callback: value => {
              const etiquetasY = { '-100': '−100 · CIN fuerte', '-50': '−50 · moderada', '-25': '−25 · débil', '0': '0', '100': '100 · muy baja', '300': '300 · baja', '700': '700 · moderada', '1200': '1200 · alta' };
              return etiquetasY[String(value)] || value;
            }
          }
        }
      }
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          ticks: { callback: value => ({'-6':'−6 · muy alta','-4':'−4 · alta','-2':'−2 · moderada','0':'0 · neutro','2':'+2 · estable'}[String(value)] || value) }
        }
      }
    }
  });
...
function dibujarSkewT(niveles, diagnostico) {
  ...
  const datosX = [...ambienteT, ...ambienteTd, ...parcela].map(p => p.x).filter(Number.isFinite);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: contexto => `${contexto.dataset.label}: ${formato(contexto.raw.temperatura, 1)} °C · ${formato(contexto.raw.presion, 0)} hPa · ${formato(contexto.raw.alturaAgl, 0)} m AGL`
          }
        }
      },
      scales: {
...
function actualizarTarjetasPerfil(contexto) {
  const ids = [
    ...
    'perfilCero', 'perfilShear6', 'perfilShear3', 'perfilInversion'
  ];
  if (!contexto) {
  ...
  if (estadoTexto) {
    ...
    estadoTexto.textContent = `Parcela superficial · CAPE ${formato(diagnostico?.cape, 0)} J/kg · CIN ${formato(diagnostico?.cin, 0)} J/kg · ${lfc} · ${el}.`;
  }
}
