...
function obtenerConfiguracionPerfil(modelo = obtenerModeloSeleccionado()) {
  if (modelo?.id === 'meteofrance_seamless') {
    return {
      ...
      maxDias: 2,
      motivo: 'seleccionado junto al modelo Météo-France'
    };
  }
...
function actualizarGraficoCapeCin(h, etiquetas, modelo) {
  ...
  if (fuenteTexto) fuenteTexto.textContent = `AROME France 2,5 km · CAPE y CIN calculados desde el mismo perfil termodinámico. Bandas MetVlc adaptadas al contexto valenciano.`;
  ...
  if (!capeDisponible && !cinDisponible) {
    ...
    canvas.hidden = true;
    aviso.textContent = 'AROME no ha devuelto un perfil válido para calcular CAPE y CIN en este punto/hora.';
    aviso.hidden = false;
    return;
  }
  ...
  aviso.hidden = true;
  const datasets = [];
  if (capeDisponible) datasets.push(barras('CAPE · AROME 2,5 km', h.cape, COLORES.cape, 'y'));
  if (cinDisponible) datasets.push(linea('CIN · AROME 2,5 km', h.convective_inhibition, COLORES.cin, 'y', 2));
...
function actualizarGraficoLi(h, etiquetas, modelo) {
  ...
  crearGrafico('li', 'graficoLi', {
    type: 'line',
    data: { labels: etiquetas, datasets: [relleno('Lifted Index · AROME 2,5 km', datos, COLORES.li, 'y')] },
    options: {
