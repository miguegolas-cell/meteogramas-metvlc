...
function aplicarIndicesInestabilidadArome(datos, perfilArome) {
  ...
  tiemposBase.forEach((tiempo, indiceBase) => {
    ...
    // CAPE principal del visor: valor nativo entregado por AROME.
    // El SBCAPE calculado se conserva únicamente dentro del diagnóstico Skew-T.
    const capeArome = numeroSeguro(perfilArome.hourly?.cape?.[indiceArome]);
    capeNativo[indiceBase] = capeArome;
    cape[indiceBase] = capeArome;
  ...
  estado.fuentesVariables.cape = 'AROME France 2,5 km · nativo';
...
function actualizarGraficoCape(h, etiquetas, modelo) {
  ...
  const ventana = recortarVentanaSerie(etiquetas, h.cape || []);
  const capeArome = ventana.datos || [];
  const disponible = serieTieneDatos(capeArome);
  const fuenteTexto = $('fuenteCape');

  if (fuenteTexto) {
    fuenteTexto.textContent = 'AROME France 2,5 km · CAPE nativo del modelo. El eje temporal termina en la última hora para la que AROME devuelve CAPE.';
  }
  ...
  if (!disponible) {
    ...
    canvas.hidden = true;
    aviso.textContent = 'AROME no ha devuelto CAPE nativo para este punto/hora.';
    aviso.hidden = false;
    return;
  }
  ...
  const valoresValidos = capeArome.filter(valor => Number.isFinite(Number(valor)));
  const capeMax = valoresValidos.length ? Math.max(...valoresValidos) : 0;
  ...
  crearGrafico('cape', 'graficoCape', {
    ...
    data: {
      ...
      datasets: [linea('CAPE nativo · AROME', capeArome, COLORES.capeNativo, 'y', 2.6)]
    },
    options: {
      ...opcionesComunes(),
      plugins: {
...
const pluginSkewT = {
  ...
  afterDatasetsDraw(chart, args, opciones) {
    ...
    niveles.forEach(([etiqueta, nivel, color], indice) => {
      ...
      ctx.setLineDash([]);
      const alturaTexto = Number.isFinite(Number(nivel.alturaAgl))
        ? ` · ${formato(nivel.alturaAgl, 0)} m AGL`
        : '';
      const texto = `${etiqueta} ${Math.round(nivel.presion)} hPa${alturaTexto}`;
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          title: { display: true, text: 'Presión / altura (hPa · m AGL)' },
          afterBuildTicks: eje => { eje.ticks = presionesTicks.map(p => ({ value: presionAY(p) })); },
          ticks: {
            padding: 5,
            callback: valor => {
              const presion = Math.round(presionDesdeY(valor));
              const alturaAgl = interpolarLogPresion(presion, diagnostico.perfil, 'alturaAgl');
              if (!Number.isFinite(Number(alturaAgl))) return `${presion} hPa`;
              return [`${presion} hPa`, `${formato(alturaAgl, 0)} m`];
            }
          },
...
function actualizarTarjetasPerfil(contexto) {
  ...
  if (estadoTexto) {
    const lfc = diagnostico?.niveles?.lfc ? `LFC ${formatearNivelTermo(diagnostico.niveles.lfc)}` : 'sin LFC';
    const el = diagnostico?.niveles?.el ? `EL ${formatearNivelTermo(diagnostico.niveles.el)}` : 'sin EL cerrado';
