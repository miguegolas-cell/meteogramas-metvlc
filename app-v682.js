...
function aplicarIndicesInestabilidadArome(datos, perfilArome) {
  ...
  tiemposBase.forEach((tiempo, indiceBase) => {
    ...
    // Usamos el CAPE calculado con la misma parcela que CIN y LI para mantener
    // coherencia interna. El campo CAPE nativo de AROME queda como referencia.
  ...
  estado.fuentesVariables.cape = 'AROME 2,5 km · perfil';
...
function actualizarGraficoCape(h, etiquetas, modelo) {
  ...
  const capeDisponible = serieTieneDatos(ventana.datos);
  const fuenteTexto = $('fuenteCape');

  if (fuenteTexto) {
    fuenteTexto.textContent = 'AROME France 2,5 km · CAPE calculado desde el perfil termodinámico. El eje temporal se limita a las horas con datos AROME disponibles.';
  }
  ...
  if (!capeDisponible) {
    ...
    canvas.hidden = true;
    aviso.textContent = 'AROME no ha devuelto un perfil válido para calcular CAPE en este punto/hora.';
    aviso.hidden = false;
    return;
  }
  ...
  const capeMax = Math.max(...ventana.datos.filter(Number.isFinite));
  ...
  crearGrafico('cape', 'graficoCape', {
    ...
    data: {
      ...
      datasets: [linea('CAPE · AROME 2,5 km', ventana.datos, COLORES.cape, 'y', 2.4)]
    },
    options: {
      ...opcionesComunes(),
      plugins: {
