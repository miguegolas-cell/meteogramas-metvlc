...
const CONFIG = {
  ...
  apiPerfilArome: 'https://api.open-meteo.com/v1/meteofrance',
  modeloPerfilArome: 'meteofrance_arome_france0025',
  archivoObservacionesAemet: 'datos/observaciones_aemet.json',
  archivoObservacionesSiar: 'datos/observaciones_siar.json',
  umbralEstacionCercanaKm: 40,
  umbralEstacionUtilKm: 80,
  nivelesPerfilArome: [1000, 950, 925, 900, 850, 800, 750, 700, 650, 600, 550, 500, 450, 400, 350, 300, 275, 250, 225, 200, 175, 150, 125, 100],
  ...
  modelos: {
    best_match: {
      id: 'best_match',
      nombre: 'Auto · resolución variable',
      corto: 'Auto',
      maxDias: 10,
      probPrecipitacion: true,
      variablesAvanzadas: ['lifted_index', 'convective_inhibition', 'boundary_layer_height'],
      ayuda: 'Selección automática de Open-Meteo. La resolución depende del modelo elegido para cada punto y variable.'
    },
    ecmwf_ifs: {
      id: 'ecmwf_ifs',
      nombre: 'ECMWF · 9 km',
      ...
      ayuda: 'ECMWF IFS HRES a 9 km. Alcance del visor limitado a 10 días.'
    },
    ncep_gfs_seamless: {
      ...
      probPrecipitacion: true,
      variablesAvanzadas: ['lifted_index', 'convective_inhibition', 'boundary_layer_height'],
      ayuda: 'NOAA GFS a ~13 km para variables de superficie; los campos en niveles de presión son más gruesos (~25 km).'
    },
    meteofrance_seamless: {
      id: 'meteofrance_seamless',
      nombre: 'AROME/ARPEGE · 2,5–11 km',
      corto: 'AROME/ARPEGE',
      maxDias: 3,
      probPrecipitacion: false,
      variablesAvanzadas: [],
      ayuda: 'Serie Météo-France: AROME 2,5 km cuando está disponible y ARPEGE Europe ~11 km como continuación. En esta interfaz se limita a 72 horas.'
    }
  }
};
...
async function consultarPrediccion(lat, lon, nombre) {
  ...
  try {
    ...
    // Guardamos ya el punto y la elevación actual para que los cálculos termodinámicos
    // de AROME no reutilicen el estado de la consulta anterior.
    estado.punto = { lat, lon, nombre };
    estado.datos = datos;

    const perfilPrincipalEsArome = obtenerConfiguracionPerfil(modelo).id === 'arome';
    const consultas = await Promise.allSettled([
      ...
      consultarPerfilSegunModelo(lat, lon, dias, modelo),
      perfilPrincipalEsArome
        ? Promise.resolve(null)
        : consultarPerfilArome(lat, lon, Math.min(2, dias))
    ]);
    ...
    const perfilSkewT = consultas[2].status === 'fulfilled' ? consultas[2].value : null;
    const perfilAromeInestabilidad = perfilPrincipalEsArome
      ? perfilSkewT
      : (consultas[3].status === 'fulfilled' ? consultas[3].value : null);

    combinarDatosAvanzados(datos, datosAvanzados);
    normalizarDatosOpcionales(datos);
    aplicarApoyoGfs(datos, apoyoGfs, modelo);
    aplicarIndicesInestabilidadArome(datos, perfilAromeInestabilidad);

    estado.datos = datos;
    estado.perfilSkewT = perfilSkewT;
    ...
    prepararSelectorPerfil(perfilSkewT);
...
function obtenerConfiguracionPerfil(modelo = obtenerModeloSeleccionado()) {
  if (modelo?.id === 'meteofrance_seamless') {
    return {
      id: 'arome',
      nombre: 'Météo-France AROME France 2,5 km',
      corto: 'AROME',
      niveles: CONFIG.nivelesPerfilArome,
      maxDias: 2,
      motivo: 'seleccionado junto a AROME/ARPEGE'
    };
  }
  ...
  return {
    ...
    nombre: 'NOAA GFS',
    corto: 'GFS',
    niveles: CONFIG.nivelesPerfilGfs,
    maxDias: 10,
    motivo: modelo?.id === 'ncep_gfs_seamless'
      ? 'seleccionado junto al modelo GFS'
      : `perfil de apoyo para ${modelo?.corto || 'el modelo principal'}`
  };
}

function obtenerMetaPerfil(perfil = estado.perfilSkewT) {
  return perfil?.metvlcPerfil || obtenerConfiguracionPerfil();
}

function actualizarEtiquetasFuentePerfil(meta) {
  const descripcion = $('descripcionPerfil');
  const insignia = $('insigniaModeloPerfil');
  const tituloHodo = $('tituloHodografa');
  const fuentePie = $('fuentePerfilPie');
  if (descripcion) {
    descripcion.textContent = `Perfil previsto por ${meta.nombre} para el punto seleccionado. No es un radiosondeo observado con globo.`;
  }
  if (insignia) insignia.textContent = `Parcela superficial ${meta.corto}`;
  if (tituloHodo) tituloHodo.textContent = `6.3 Hodógrafa ${meta.corto}`;
  if (fuentePie) fuentePie.textContent = meta.nombre;
}

async function consultarPerfilSegunModelo(lat, lon, dias, modelo) {
  const configuracion = obtenerConfiguracionPerfil(modelo);
  const datosPerfil = configuracion.id === 'arome'
    ? await consultarPerfilArome(lat, lon, Math.min(dias, configuracion.maxDias))
    : await consultarPerfilGfs(lat, lon, Math.min(dias, configuracion.maxDias));
  datosPerfil.metvlcPerfil = configuracion;
  return datosPerfil;
}
...
async function consultarPerfilGfs(lat, lon, dias) {
  ...
  const superficie = resultados[0];
  if (superficie.status !== 'fulfilled' || !superficie.value?.hourly?.time?.length) {
    throw new Error('GFS no ha devuelto las variables de superficie necesarias para el perfil.');
  }
  ...
  return {
    ...
    metvlcPerfil: obtenerConfiguracionPerfil(CONFIG.modelos.ncep_gfs_seamless)
  };
}
...
async function solicitarPerfilGfsGrupo(lat, lon, dias, variables) {
  ...
  url.search = new URLSearchParams({
    ...
    longitude: lon.toFixed(5),
    hourly: variables.join(','),
    forecast_days: String(Math.min(dias, 10)),
    timezone: CONFIG.zonaHoraria,
    wind_speed_unit: 'ms'
  });
...
async function consultarPerfilArome(lat, lon, dias) {
  const niveles = CONFIG.nivelesPerfilArome;
  const grupos = [
    ['temperature_2m', 'dew_point_2m', 'relative_humidity_2m', 'wind_speed_10m', 'wind_direction_10m', 'surface_pressure', 'cape']
  ];

  // El perfil AROME se divide en bloques para evitar URLs largas y respuestas parciales.
  trocear(niveles, 10).forEach(bloque => {
    grupos.push(bloque.flatMap(nivel => [
      `temperature_${nivel}hPa`,
      `relative_humidity_${nivel}hPa`,
  ...
  const resultados = await Promise.allSettled(
    grupos.map(variables => solicitarPerfilAromeRobusto(lat, lon, dias, variables))
  );
  const superficie = resultados[0];
  if (superficie.status !== 'fulfilled' || !superficie.value?.hourly?.time?.length) {
    throw new Error('AROME no ha devuelto las variables de superficie necesarias o el punto queda fuera de su dominio.');
  }
  ...
  return {
    ...
    hourly,
    gruposFallidos: resultados.filter(resultado => resultado.status === 'rejected').length,
    metvlcPerfil: obtenerConfiguracionPerfil(CONFIG.modelos.meteofrance_seamless)
  };
}
...
async function solicitarPerfilAromeRobusto(lat, lon, dias, variables) {
  try {
    return await solicitarPerfilAromeGrupo(lat, lon, dias, variables);
  } catch (error) {
    if (variables.length <= 2) throw error;
    const mitad = Math.ceil(variables.length / 2);
    const resultados = await Promise.allSettled([
      solicitarPerfilAromeRobusto(lat, lon, dias, variables.slice(0, mitad)),
      solicitarPerfilAromeRobusto(lat, lon, dias, variables.slice(mitad))
    ]);
    const validos = resultados
      .filter(resultado => resultado.status === 'fulfilled' && resultado.value?.hourly?.time?.length)
      .map(resultado => resultado.value);
...
async function solicitarPerfilAromeGrupo(lat, lon, dias, variables) {
  const url = new URL(CONFIG.apiPerfilArome);
  url.search = new URLSearchParams({
    latitude: lat.toFixed(5),
    longitude: lon.toFixed(5),
    hourly: variables.join(','),
    forecast_days: String(Math.min(dias, 2)),
    models: CONFIG.modeloPerfilArome,
    timezone: CONFIG.zonaHoraria,
    wind_speed_unit: 'ms'
  });
  ...
  if (!respuesta.ok) {
    let detalle = '';
    try { detalle = (await respuesta.json()).reason || ''; } catch { detalle = ''; }
    throw new Error(detalle || `Perfil AROME: HTTP ${respuesta.status}`);
  }
  const datos = await respuesta.json();
  if (!datos?.hourly?.time?.length) throw new Error('El bloque del perfil AROME no contiene horas.');
  return datos;
}
...
function normalizarDatosOpcionales(datos) {
  ...
  const opcionalesHorarias = [
    'precipitation_probability', 'temperature_850hPa', 'cape', 'cape_native_arome', 'lifted_index',
...
function aplicarIndicesInestabilidadArome(datos, perfilArome) {
  const tiemposBase = datos?.hourly?.time || [];
  const vacio = Array(tiemposBase.length).fill(null);
  if (!perfilArome?.hourly?.time?.length) {
    datos.hourly.cape = [...vacio];
    datos.hourly.cape_native_arome = [...vacio];
    datos.hourly.convective_inhibition = [...vacio];
    datos.hourly.lifted_index = [...vacio];
    estado.fuentesVariables.cape = 'AROME no disponible';
    estado.fuentesVariables.convective_inhibition = 'AROME no disponible';
    estado.fuentesVariables.lifted_index = 'AROME no disponible';
    recalcularCapeDiarioArome(datos);
    return;
  }

  const mapaHoras = new Map(perfilArome.hourly.time.map((t, i) => [t, i]));
  const cape = [...vacio];
  const capeNativo = [...vacio];
  const cin = [...vacio];
  const li = [...vacio];
  const elevacion = Number(datos.elevation ?? perfilArome.elevation ?? 0);

  tiemposBase.forEach((tiempo, indiceBase) => {
    const indiceArome = mapaHoras.get(tiempo);
    if (indiceArome == null) return;

    // Conservamos el CAPE nativo que entrega AROME para compararlo directamente
    // con el SBCAPE calculado por MetVlc a partir del perfil termodinámico.
    capeNativo[indiceBase] = numeroSeguro(perfilArome.hourly?.cape?.[indiceArome]);

    const niveles = extraerNivelesPerfilConElevacion(perfilArome, indiceArome, elevacion);
    if (niveles.length < 5) return;
    const diagnostico = calcularDiagnosticoTermodinamico(perfilArome, indiceArome, niveles, elevacion);
    if (!diagnostico) return;

    cape[indiceBase] = numeroSeguro(diagnostico.cape);
    cin[indiceBase] = numeroSeguro(diagnostico.cin);
    const t500 = interpolarLogPresion(500, diagnostico.perfil, 'temperatura');
    const parcela500 = interpolarLogPresion(500, diagnostico.perfil, 'parcela');
    li[indiceBase] = t500 != null && parcela500 != null ? t500 - parcela500 : null;
  });

  datos.hourly.cape = cape;
  datos.hourly.cape_native_arome = capeNativo;
  datos.hourly.convective_inhibition = cin;
  datos.hourly.lifted_index = li;
  estado.fuentesVariables.cape = 'AROME 2,5 km · nativo + SBCAPE MetVlc';
  estado.fuentesVariables.convective_inhibition = 'AROME 2,5 km · perfil';
  estado.fuentesVariables.lifted_index = 'AROME 2,5 km · perfil';
  recalcularCapeDiarioArome(datos);
}

function recalcularCapeDiarioArome(datos) {
  if (!datos?.daily?.time?.length) return;
  datos.daily.cape_max = datos.daily.time.map(fecha => {
    const valores = (datos.hourly.time || [])
      .map((tiempo, i) => tiempo.slice(0, 10) === fecha ? numeroSeguro(datos.hourly.cape?.[i]) : null)
...
function actualizarGraficos(datos) {
  ...
  actualizarGraficoCape(h, etiquetas, modelo);

  actualizarGraficoCin(h, etiquetas, modelo);
...
function recortarVentanaSerie(etiquetas, datos) {
  if (!Array.isArray(datos) || !Array.isArray(etiquetas)) {
    return { etiquetas: [], datos: [] };
  }

  const limite = Math.min(etiquetas.length, datos.length);
  let primero = -1;
  let ultimo = -1;

  for (let i = 0; i < limite; i += 1) {
    const valor = datos[i];
    // Importante: Number(null) === 0. Si no excluimos explícitamente null,
    // las horas posteriores al horizonte AROME se interpretan como datos válidos
    // y el eje X se alarga hasta el horizonte completo del modelo principal.
  ...
  if (primero < 0 || ultimo < 0) return { etiquetas: [], datos: [] };

  return {
    etiquetas: etiquetas.slice(primero, ultimo + 1),
    datos: datos.slice(primero, ultimo + 1)
  };
}

function recortarVentanaSeries(etiquetas, series) {
  if (!Array.isArray(etiquetas) || !Array.isArray(series) || !series.length) {
    return { etiquetas: [], series: [] };
  }

  const limite = Math.min(
    etiquetas.length,
    ...series.map(serie => Array.isArray(serie) ? serie.length : 0)
  );
  let primero = -1;
  let ultimo = -1;

  for (let i = 0; i < limite; i += 1) {
    const hayDato = series.some(serie => {
      const valor = serie?.[i];
  ...
  if (primero < 0 || ultimo < 0) return { etiquetas: [], series: series.map(() => []) };

  return {
    etiquetas: etiquetas.slice(primero, ultimo + 1),
    series: series.map(serie => serie.slice(primero, ultimo + 1))
  };
}

function actualizarGraficoCape(h, etiquetas, modelo) {
  const canvas = $('graficoCape');
  const aviso = $('sinDatosCape');
  const comparacion = recortarVentanaSeries(
    etiquetas,
    [h.cape_native_arome || [], h.cape || []]
  );
  const capeNativo = comparacion.series[0] || [];
  const capeCalculado = comparacion.series[1] || [];
  const nativoDisponible = serieTieneDatos(capeNativo);
  const calculadoDisponible = serieTieneDatos(capeCalculado);
  const fuenteTexto = $('fuenteCape');

  if (fuenteTexto) {
    fuenteTexto.textContent = 'Comparación de control · AROME France 2,5 km: CAPE nativo del modelo (línea continua) frente a SBCAPE calculado por MetVlc con parcela superficial (línea discontinua). El eje temporal termina en la última hora AROME disponible.';
  }

  if (!nativoDisponible && !calculadoDisponible) {
    if (estado.graficos.cape) {
      estado.graficos.cape.destroy();
      delete estado.graficos.cape;
    }
    canvas.hidden = true;
    aviso.textContent = 'AROME no ha devuelto CAPE nativo ni un perfil válido para calcular SBCAPE en este punto/hora.';
    aviso.hidden = false;
    return;
  }
  ...
  const todos = [...capeNativo, ...capeCalculado].filter(valor => Number.isFinite(valor));
  const capeMax = todos.length ? Math.max(...todos) : 0;
  const maxY = Math.max(2000, Math.ceil(capeMax / 100) * 100);
  const datasets = [];
  if (nativoDisponible) {
    datasets.push(linea('CAPE nativo · AROME', capeNativo, COLORES.capeNativo, 'y', 2.5));
  }
  if (calculadoDisponible) {
    datasets.push(linea('SBCAPE calculado · MetVlc', capeCalculado, COLORES.cape, 'y', 2.2, true));
  }

  crearGrafico('cape', 'graficoCape', {
    type: 'line',
    data: {
      labels: comparacion.etiquetas,
      datasets
    },
    options: {
      ...opcionesComunes(),
      plugins: {
        ...
        bandasOperativas: {
          bandas: [
            { desde: 0, hasta: 100, etiqueta: 'Residual', color: 'rgba(96,155,109,.055)' },
            { desde: 100, hasta: 300, etiqueta: 'Baja', color: 'rgba(116,177,110,.065)' },
            { desde: 300, hasta: 700, etiqueta: 'Moderada', color: 'rgba(226,184,72,.075)' },
            { desde: 700, hasta: 1200, etiqueta: 'Alta', color: 'rgba(226,125,55,.08)' },
            { desde: 1200, hasta: 2000, etiqueta: 'Muy alta', color: 'rgba(211,84,49,.085)' },
            { desde: 2000, hasta: maxY, etiqueta: 'Extrema', color: 'rgba(172,45,48,.095)' }
          ]
        },
        tooltip: {
          enabled: true,
          callbacks: {
            afterBody: items => {
              ...
              const i = items[0].dataIndex;
              const nativo = numeroSeguro(capeNativo[i]);
              const calculado = numeroSeguro(capeCalculado[i]);
              if (nativo == null || calculado == null) return '';
              const diferencia = calculado - nativo;
              const porcentaje = nativo !== 0 ? (diferencia / nativo) * 100 : null;
              const signo = diferencia > 0 ? '+' : '';
              return porcentaje == null
                ? `Diferencia MetVlc − nativo: ${signo}${formato(diferencia, 0)} J/kg`
                : `Diferencia MetVlc − nativo: ${signo}${formato(diferencia, 0)} J/kg (${signo}${formato(porcentaje, 0)} %)`;
            }
          }
        }
      },
      scales: {
        x: escalaXInestabilidad(comparacion.etiquetas.length),
        y: {
          min: 0,
          max: maxY,
          title: { display: true, text: 'CAPE / SBCAPE (J/kg)' },
          afterBuildTicks: escala => {
            const candidatos = [0, 100, 300, 700, 1200, 2000, maxY];
            escala.ticks = [...new Set(candidatos.filter(v => v >= 0 && v <= maxY))]
              .sort((a, b) => a - b)
              .map(value => ({ value }));
          },
...
function actualizarGraficoCin(h, etiquetas, modelo) {
  const canvas = $('graficoCin');
  const aviso = $('sinDatosCin');
  const ventana = recortarVentanaSerie(etiquetas, h.convective_inhibition);
  const cinDisponible = serieTieneDatos(ventana.datos);
  const fuenteTexto = $('fuenteCin');

  if (fuenteTexto) {
    fuenteTexto.textContent = 'AROME France 2,5 km · CIN calculado desde el mismo perfil termodinámico que CAPE. Se representa con signo negativo: 0 J/kg = sin inhibición; cuanto más negativo, mayor inhibición. El eje temporal se limita a las horas con datos AROME disponibles.';
  }

  if (!cinDisponible) {
    if (estado.graficos.cin) {
      estado.graficos.cin.destroy();
      delete estado.graficos.cin;
    }
    canvas.hidden = true;
    aviso.textContent = 'AROME no ha devuelto un perfil válido para calcular CIN en este punto/hora.';
    aviso.hidden = false;
    return;
  }
  ...
  const cinMin = Math.min(...ventana.datos.filter(Number.isFinite));
  const minY = Math.min(-125, Math.floor(cinMin / 25) * 25);

  crearGrafico('cin', 'graficoCin', {
    type: 'line',
    data: {
      labels: ventana.etiquetas,
      datasets: [linea('CIN · AROME 2,5 km', ventana.datos, COLORES.cin, 'y', 2)]
    },
    options: {
      ...opcionesComunes(),
      plugins: {
        ...
        bandasOperativas: {
          bandas: [
            { desde: minY, hasta: -125, etiqueta: 'Muy fuerte', color: 'rgba(120,57,38,.09)' },
            { desde: -125, hasta: -75, etiqueta: 'Fuerte', color: 'rgba(160,79,41,.085)' },
            { desde: -75, hasta: -35, etiqueta: 'Moderada', color: 'rgba(196,119,49,.08)' },
            { desde: -35, hasta: -15, etiqueta: 'Débil', color: 'rgba(222,163,77,.075)' },
            { desde: -15, hasta: 0, etiqueta: 'Escasa', color: 'rgba(239,205,126,.07)' }
          ]
        }
      },
      scales: {
        x: escalaXInestabilidad(ventana.etiquetas.length),
        y: {
          min: minY,
          max: 0,
          title: { display: true, text: 'CIN (J/kg · 0 = sin inhibición)' },
          grid: {
            color: contexto => Number(contexto.tick.value) === 0
              ? 'rgba(25,45,60,.62)'
              : 'rgba(127,151,170,.18)',
            lineWidth: contexto => Number(contexto.tick.value) === 0 ? 2 : 1
          },
          afterBuildTicks: escala => {
            const candidatos = [minY, -250, -200, -150, -125, -100, -75, -50, -35, -15, 0];
            const visibles = [...new Set(candidatos.filter(v => v >= minY && v <= 0))]
              .sort((a, b) => a - b);
            if (!visibles.includes(0)) visibles.push(0);
            escala.ticks = visibles
              .sort((a, b) => a - b)
              .map(value => ({ value }));
          },
          ticks: {
...
function actualizarGraficoLi(h, etiquetas, modelo) {
  ...
  const ventana = recortarVentanaSerie(etiquetas, h.lifted_index);
  const datos = ventana.datos;
  if (!serieTieneDatos(datos)) {
    if (estado.graficos.li) { estado.graficos.li.destroy(); delete estado.graficos.li; }
    canvas.hidden = true;
    aviso.textContent = 'AROME no ha devuelto un perfil válido para calcular Lifted Index en este punto/hora.';
    aviso.hidden = false;
    return;
  }
  canvas.hidden = false;
  ...
  crearGrafico('li', 'graficoLi', {
    type: 'line',
    data: { labels: ventana.etiquetas, datasets: [relleno('Lifted Index · AROME 2,5 km', datos, COLORES.li, 'y')] },
    options: {
...
function escalaXInestabilidad(numeroHoras) {
  const horas = Math.max(1, Number(numeroHoras) || 1);
  const maxTicks = horas <= 30 ? 10 : horas <= 54 ? 12 : 14;
  return {
    grid: { display: false },
    offset: false,
    ticks: {
...
function prepararSelectorPerfil(perfil) {
  ...
  if (!perfil?.hourly?.time?.length) {
    ...
    const esperado = obtenerConfiguracionPerfil();
    actualizarEtiquetasFuentePerfil(esperado);
    textoHora.textContent = `Perfil ${esperado.corto} no disponible`;
    estadoPerfil.textContent = `No ha sido posible cargar los niveles de presión de ${esperado.nombre} para este punto.`;
    mostrarPerfilSinDatos(`El perfil vertical ${esperado.corto} no está disponible para este punto u hora.`);
    return;
  }
  ...
  const meta = obtenerMetaPerfil(perfil);
  actualizarEtiquetasFuentePerfil(meta);
  estadoPerfil.textContent = `Fuente: ${meta.nombre} · ${meta.motivo} · desplaza la barra para cambiar la hora del perfil.`;
  actualizarTextoHoraPerfil();
...
function actualizarTextoHoraPerfil() {
  const selector = $('horaPerfil');
  const textoHora = $('horaPerfilTexto');
  const tiempos = estado.perfilSkewT?.hourly?.time;
...
function actualizarPerfilVertical() {
  const perfil = estado.perfilSkewT;
  const selector = $('horaPerfil');
  ...
  const meta = obtenerMetaPerfil(perfil);
  $('estadoPerfil').textContent = `${meta.nombre} · ${formatearFechaHora(perfil.hourly.time[indice])} · T: ${nivelesTemperatura} niveles · Td: ${nivelesRocio} niveles${fallos ? ` · ${fallos} bloque(s) reintentables sin respuesta` : ''}.`;
}

function extraerNivelesPerfil(perfil, indice) {
  const elevacion = Number(estado.datos?.elevation ?? perfil.elevation ?? 0);
  return extraerNivelesPerfilConElevacion(perfil, indice, elevacion);
}

function extraerNivelesPerfilConElevacion(perfil, indice, elevacion) {
  const nivelesConfigurados = obtenerMetaPerfil(perfil).niveles || CONFIG.nivelesPerfilGfs;
  return nivelesConfigurados.map(presion => {
...
function calcularDiagnosticoTermodinamico(perfil, indice, niveles, elevacionForzada = null) {
  const elevacion = elevacionForzada == null ? Number(estado.datos?.elevation ?? perfil.elevation ?? 0) : Number(elevacionForzada);
  ...
  const lcl = completarNivelPorPresion(lclBase.presion, perfilDenso, elevacion);
...
function completarNivelPorPresion(presion, perfil, elevacionForzada = null) {
  ...
  const elevacion = elevacionForzada == null ? Number(estado.datos?.elevation ?? 0) : Number(elevacionForzada);
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          ticks: {
            callback: valor => niveles.some(nivel => nivel.presion === Number(valor)) ? valor : ''
          }
        }
      }
    }
...
function urlsArchivoObservaciones(ruta) {
  const urls = [];
  const host = window.location.hostname.toLowerCase();
  const partesRuta = window.location.pathname.split('/').filter(Boolean);

  // En GitHub Pages leemos primero la rama main para evitar retrasos de publicación
  // de JSON actualizados por GitHub Actions.
  if (host.endsWith('.github.io')) {
    const propietario = host.split('.')[0];
    const repositorio = partesRuta[0];
    if (propietario && repositorio) {
      urls.push(`https://raw.githubusercontent.com/${propietario}/${repositorio}/main/${ruta}`);
    }
  }
...
async function descargarJsonObservaciones(ruta, validador) {
  const errores = [];
  for (const url of urlsArchivoObservaciones(ruta)) {
    try {
      const separador = url.includes('?') ? '&' : '?';
      const respuesta = await fetch(`${url}${separador}t=${Date.now()}`, { cache: 'no-store' });
      if (!respuesta.ok) throw new Error(`HTTP ${respuesta.status}`);
      const archivo = await respuesta.json();
      if (!validador(archivo)) throw new Error('JSON sin estaciones válidas');
      return archivo;
    } catch (error) {
...
async function descargarArchivoAemet() {
  return descargarJsonObservaciones(
    CONFIG.archivoObservacionesAemet,
    archivo => Array.isArray(archivo?.stations) && archivo.stations.length > 0
  );
}

async function descargarArchivoSiar() {
  return descargarJsonObservaciones(
    CONFIG.archivoObservacionesSiar,
    archivo => Array.isArray(archivo?.estaciones) && archivo.estaciones.length > 0
  );
}

function normalizarEstacionAemet(estacion) {
  return {
    red: 'AEMET',
    id: estacion.id,
    name: estacion.name,
    lat: numeroSeguro(estacion.lat),
    lon: numeroSeguro(estacion.lon),
    alt: numeroSeguro(estacion.alt),
    time: estacion.time || null,
    horaTexto: null,
    temp: numeroSeguro(estacion.temp),
    rh: numeroSeguro(estacion.rh),
    dewpoint: numeroSeguro(estacion.dewpoint),
    wind_speed: numeroSeguro(estacion.wind_speed), // m/s
    wind_direction: numeroSeguro(estacion.wind_direction),
    gust: numeroSeguro(estacion.gust), // m/s
    precip: numeroSeguro(estacion.precip),
    dpv: null
  };
}

function normalizarHoraSiar(fecha, hora) {
  const textoHora = String(hora ?? '').padStart(4, '0');
  if (!fecha || !/^\d{4}$/.test(textoHora)) return null;
  return `${String(fecha).slice(0, 10)} · ${textoHora.slice(0, 2)}:${textoHora.slice(2, 4)}`;
}

function normalizarEstacionSiar(estacion) {
  return {
    red: 'SIAR',
    id: estacion.codigo,
    name: estacion.nombre || estacion.municipio || estacion.codigo,
    lat: numeroSeguro(estacion.latitud),
    lon: numeroSeguro(estacion.longitud),
    alt: numeroSeguro(estacion.altitud),
    time: null,
    horaTexto: normalizarHoraSiar(estacion.fecha, estacion.hora),
    temp: numeroSeguro(estacion.temperatura),
    rh: numeroSeguro(estacion.humedad),
    dewpoint: numeroSeguro(estacion.punto_rocio),
    wind_speed: numeroSeguro(estacion.viento), // SiAR publica Vel. Viento en m/s
    wind_direction: numeroSeguro(estacion.direccion_viento),
    gust: null,
    precip: numeroSeguro(estacion.precipitacion),
    dpv: numeroSeguro(estacion.dpv)
  };
}

function diferenciaAltitudEstacion(estacion) {
  const altEstacion = numeroSeguro(estacion.alt);
  const altPunto = numeroSeguro(estado.datos?.elevation);
  return altEstacion != null && altPunto != null ? altEstacion - altPunto : null;
}

function puntuacionRepresentatividad(estacion) {
  // Distancia domina la selección. La diferencia de altitud añade una penalización
  // moderada: 100 m de diferencia equivalen aproximadamente a 3 km adicionales.
  const distancia = numeroSeguro(estacion.distancia);
  if (distancia == null) return Infinity;
  const deltaAlt = diferenciaAltitudEstacion(estacion);
  const penalizacionAltitud = deltaAlt == null ? 0 : Math.abs(deltaAlt) * 0.03;
  return distancia + penalizacionAltitud;
}

function textoRed(red) {
  return red === 'SIAR' ? 'SiAR' : 'AEMET';
}

function claseRed(red) {
  return red === 'SIAR' ? 'red-siar' : 'red-aemet';
}

function textoHoraEstacion(estacion) {
  if (estacion.horaTexto) return estacion.horaTexto;
  if (estacion.time) return formatearHoraObservacion(estacion.time);
  return '—';
}
...
async function actualizarObservacionesActuales(lat, lon, datosModelo) {
  ...
  if ($('estadoObservaciones')) $('estadoObservaciones').textContent = 'Buscando AEMET + SiAR…';
  if ($('tablaObservaciones')) $('tablaObservaciones').innerHTML = '<tr><td colspan="10">Consultando observaciones AEMET y SiAR…</td></tr>';

  const [resultadoAemet, resultadoSiar] = await Promise.allSettled([
    descargarArchivoAemet(),
    descargarArchivoSiar()
  ]);
  ...
  if (resultadoAemet.status === 'fulfilled') {
    estaciones.push(...resultadoAemet.value.stations.map(normalizarEstacionAemet));
    fuentes.push('AEMET');
  } else {
    errores.push(`AEMET: ${resultadoAemet.reason?.message || 'no disponible'}`);
  }

  if (resultadoSiar.status === 'fulfilled') {
    estaciones.push(...resultadoSiar.value.estaciones.map(normalizarEstacionSiar));
    fuentes.push('SiAR');
  } else {
    errores.push(`SiAR: ${resultadoSiar.reason?.message || 'no disponible'}`);
  }
  ...
  const ordenadas = estaciones
    ...
    .map(estacion => ({ ...estacion, representatividad: puntuacionRepresentatividad(estacion) }))
    .sort((a, b) => a.representatividad - b.representatividad);

  estado.observaciones = ordenadas;

  if (!ordenadas.length) {
    console.warn('Observaciones AEMET/SiAR no disponibles:', errores.join(' | '));
    renderizarFallbackModelo(datosModelo, 'No se pudieron cargar observaciones AEMET ni SiAR actualizadas.');
    return;
  }

  renderizarObservaciones(ordenadas, datosModelo, {
    fuentes,
    errores,
    generadoAemet: resultadoAemet.status === 'fulfilled' ? resultadoAemet.value.generated_at : null,
    generadoSiar: resultadoSiar.status === 'fulfilled' ? resultadoSiar.value.actualizado : null
  });
}

function renderizarObservaciones(estaciones, datosModelo, meta = {}) {
  const representativas = estaciones.slice(0, 3);
  const principal = representativas[0];
  if (!principal) {
    renderizarFallbackModelo(datosModelo, 'No hay estaciones AEMET o SiAR utilizables.');
    return;
  }
  ...
  const diferenciaAltitud = diferenciaAltitudEstacion(principal);
  const dpv = principal.dpv ?? calcularDpvDesdeTempHr(principal.temp, principal.rh);
  ...
  const red = textoRed(principal.red);

  $('obsNombre').textContent = principal.name || principal.id || `Estación ${red}`;
  $('obsDistancia').textContent = `${red} · ${formato(principal.distancia, 1)} km · ${formato(principal.alt, 0)} m s.n.m.${diferenciaAltitud == null ? '' : ` · Δalt ${diferenciaAltitud >= 0 ? '+' : ''}${formato(diferenciaAltitud, 0)} m`}`;
  ...
  $('obsViento').textContent = viento == null ? '—' : `${formato(viento * 3.6, 0)} km/h${racha == null ? '' : ` · racha ${formato(racha * 3.6, 0)} km/h`}`;
  ...
  $('obsHora').textContent = `Observación ${red}: ${textoHoraEstacion(principal)}`;

  const estadoObs = $('estadoObservaciones');
  if (estadoObs) {
    const redes = meta.fuentes?.length ? meta.fuentes.join(' + ') : red;
    estadoObs.textContent = cercana
      ? `${redes} · ${formato(principal.distancia, 1)} km`
      : util
        ? `${redes} · estación distante`
        : `${redes} · baja representatividad`;
    estadoObs.className = `insignia insignia-observaciones ${cercana ? 'obs-cercana' : util ? 'obs-distante' : 'obs-no-representativa'}`;
  }

  $('tablaObservaciones').innerHTML = representativas.map(estacion => {
    const dpvEst = estacion.dpv ?? calcularDpvDesdeTempHr(estacion.temp, estacion.rh);
    ...
    const deltaAlt = diferenciaAltitudEstacion(estacion);
    const redEst = textoRed(estacion.red);
    return `<tr>
      <td><strong>${escapar(estacion.name || estacion.id || redEst)}</strong><small><span class="red-estacion ${claseRed(estacion.red)}">${redEst}</span> ${escapar(estacion.id || '')}</small></td>
      <td>${formato(estacion.distancia, 1)} km</td>
      <td>${formato(estacion.alt, 0)} m${deltaAlt == null ? '' : `<small>Δ ${deltaAlt >= 0 ? '+' : ''}${formato(deltaAlt, 0)} m</small>`}</td>
      <td>${escapar(textoHoraEstacion(estacion))}</td>
  ...
  if (aviso) {
    const incidencias = meta.errores?.length ? ` Fuente no disponible: ${escapar(meta.errores.join(' · '))}.` : '';
    if (cercana && !incidencias) {
      aviso.hidden = true;
    } else {
      aviso.hidden = false;
      const textoDistancia = cercana
        ? ''
        : `<strong>${util ? 'Estación distante:' : 'Representatividad limitada:'}</strong> la mejor estación disponible está a ${formato(principal.distancia, 1)} km. `;
      aviso.innerHTML = `${textoDistancia}${incidencias}${!cercana ? ` ${textoModeloActual(datosModelo)}` : ''}`;
    }
  }
}
