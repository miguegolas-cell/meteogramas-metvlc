...
function actualizarGraficoCape(h, etiquetas, modelo) {
  ...
  crearGrafico('cape', 'graficoCape', {
    ...
    data: {
      ...
      datasets: [barras('CAPE · AROME 2,5 km', ventana.datos, COLORES.cape, 'y')]
    },
    options: {
      ...opcionesComunes(),
      plugins: {
...
function actualizarGraficoCin(h, etiquetas, modelo) {
  ...
  if (fuenteTexto) {
    fuenteTexto.textContent = 'AROME France 2,5 km · CIN calculado desde el mismo perfil termodinámico que CAPE. El eje temporal se limita a las horas con datos AROME disponibles.';
  }
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          min: minY,
          max: 0,
          title: { display: true, text: 'CIN (J/kg)' },
          grid: { color: contexto => contexto.tick.value === 0 ? 'rgba(25,45,60,.5)' : 'rgba(127,151,170,.18)' },
          afterBuildTicks: escala => {
            const candidatos = [minY, -200, -125, -75, -35, -15, 0];
            escala.ticks = [...new Set(candidatos.filter(v => v >= minY && v <= 0))]
              .sort((a, b) => a - b)
              .map(value => ({ value }));
          },
          ticks: { callback: value => Number(value).toLocaleString('es-ES') }
