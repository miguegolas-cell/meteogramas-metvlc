...
function urlsObservacionesAemet() {
  const urls = [];
  const host = window.location.hostname.toLowerCase();
  const partesRuta = window.location.pathname.split('/').filter(Boolean);

  // Los commits realizados por GitHub Actions no siempre regeneran inmediatamente
  // la copia publicada por GitHub Pages. En github.io leemos el JSON directamente
  // desde la rama main del mismo repositorio para recibir la actualización horaria.
  if (host.endsWith('.github.io')) {
    const propietario = host.split('.')[0];
    const repositorio = partesRuta[0];
    if (propietario && repositorio) {
      urls.push(`https://raw.githubusercontent.com/${propietario}/${repositorio}/main/${CONFIG.archivoObservacionesAemet}`);
    }
  }

  urls.push(CONFIG.archivoObservacionesAemet);
  return [...new Set(urls)];
}

async function descargarArchivoObservaciones() {
  const errores = [];
  for (const url of urlsObservacionesAemet()) {
    try {
      ...
      if (!estaciones.length) {
        throw new Error(`JSON sin estaciones (${archivo?.station_count ?? 0})`);
      }
      return archivo;
    } catch (error) {
...
async function actualizarObservacionesActuales(lat, lon, datosModelo) {
  ...
  if ($('tablaObservaciones')) $('tablaObservaciones').innerHTML = '<tr><td colspan="10">Consultando observaciones AEMET…</td></tr>';
  try {
    const archivo = await descargarArchivoObservaciones();
    const estaciones = archivo.stations;
    ...
    renderizarFallbackModelo(datosModelo, 'No se pudieron cargar las observaciones AEMET actualizadas.');
  }
}
